...
def draw_zoom_window(ax, x0, y0):
    """(x0,y0)を左上座標とする位置に10x10のポリゴンを描画する"""

    ax.plot([x0, x0, x0 + 10, x0 + 10, x0], [y0, y0 + 10, y0 + 10, y0, y0], "-k", lw=3)
    ax.plot(
        [x0, x0, x0 + 10, x0 + 10, x0],
        [y0, y0 + 10, y0 + 10, y0, y0],
        "-",
        color="dodgerblue",
        lw=2,
    )
...
def plot_feature_loc(da, ax, q=[0, 1, 10, 25, 50, 75, 90, 99, 100]):
    ...
    pes = [path_effects.withStroke(linewidth=2, foreground="w")]
    markerlist = ["min", "$01$", "$10$", "$25$", "$50$", "$75$", "$90$", "$99$", "max"]
    zlist = list(zip(xs, ys))
    for i, (x, y) in enumerate(zlist):
        ax.text(y, x, markerlist[i], path_effects=pes)
    return
...
def load_n_combine_df(
    ...
    df_ir = pd.read_csv(
        path_to_data + "IR_stats_master.csv",
        index_col=0,
        low_memory=False,
        parse_dates=True,
    )
    df_wv = pd.read_csv(
        path_to_data + "WV_stats_master.csv",
        index_col=0,
        low_memory=False,
        parse_dates=True,
    )
    df_vis = pd.read_csv(
        path_to_data + "VIS_stats_master.csv",
        index_col=0,
        low_memory=False,
        parse_dates=True,
    )
    df_vil = pd.read_csv(
        path_to_data + "VIL_stats_master.csv",
        index_col=0,
        low_memory=False,
        parse_dates=True,
    )
    df_li = pd.read_csv(
        path_to_data + "LI_stats_master.csv",
        index_col=0,
        low_memory=False,
        parse_dates=True,
    )
    ...
    # 検証とテストが同じイベントとならないように隔週で分割する
    va = np.arange(22, 52, 2)  # 22週目（6/1を含む週）からの偶数週
    te = np.arange(23, 53, 2)  # 23週目からの奇数週
    ...
    X_train = np.hstack(
        [
            df_ir_tr.to_numpy() * 1e-2,
            df_wv_tr.to_numpy() * 1e-2,
            df_vis_tr.to_numpy() * 1e-4,
            get_right_units_vil(df_vil_tr.to_numpy()),
        ]
    )
    X_validate = np.hstack(
        [
            df_ir_va.to_numpy() * 1e-2,
            df_wv_va.to_numpy() * 1e-2,
            df_vis_va.to_numpy() * 1e-4,
            get_right_units_vil(df_vil_va.to_numpy()),
        ]
    )
    X_test = np.hstack(
        [
            df_ir_te.to_numpy() * 1e-2,
            df_wv_te.to_numpy() * 1e-2,
            df_vis_te.to_numpy() * 1e-4,
            get_right_units_vil(df_vil_te.to_numpy()),
        ]
    )

    # 引数でインデックスとして指定された特徴量を取り出す（デフォルトは全特徴量=9*4）
...
def boxbin(
    ...
    for i in np.arange(1, xedge.shape[0]):
        midpoints[i - 1] = xedge[i - 1] + (np.abs(xedge[i] - xedge[i - 1])) / 2.0
...
def make_colorbar(ax, vmin, vmax, cmap):
    ...
    cb = matplotlib.colorbar.ColorbarBase(
        ax, cmap=cmap, norm=cNorm, orientation="vertical", extend="max"
    )
    return cb
